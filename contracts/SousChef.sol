...
contract SousChef {
    ...
    // Info of each user.
    struct UserInfo {
        uint256 amount;   // How many churu tokens the user has provided.
        uint256 rewardDebt;  // Reward debt. See explanation below.
        uint256 rewardPending;
        //
        // We do some fancy math here. Basically, any point in time, the amount of churus
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accRewardPerShare) - user.rewardDebt + user.rewardPending
        //
        // Whenever a user deposits or withdraws churu tokens to a pool. Here's what happens:
    ...
    // Info of each user that stakes churu tokens.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        uint256 stakedSupply = churu.balanceOf(address(this));
    ...
    function updatePool() public {
        ...
        uint256 churuSupply = churu.balanceOf(address(this));
        if (churuSupply == 0) {
        ...
        poolInfo.accRewardPerShare = poolInfo.accRewardPerShare.add(tokenReward.mul(1e12).div(churuSupply));
        poolInfo.lastRewardBlock = block.number;
    }
    ...
    // Withdraw churu tokens from SousChef.
    function withdraw(uint256 _amount) public {
        ...
        updatePool();
        churu.safeTransfer(address(msg.sender), _amount);
    ...
    function emergencyWithdraw() public {
        UserInfo storage user = userInfo[msg.sender];
        churu.safeTransfer(address(msg.sender), user.amount);
